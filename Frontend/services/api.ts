...
export type CourseStatus = "Online" | "Offline" | "Hybrid";
...
export interface Course {
  ...
  name: string;
  description: string;
  status: CourseStatus | CourseStatus[]; // ← UPDATED: Can be string OR array for backward compatibility
  language: string;
...
class CourseAPI {
   ...
   * NOTE: For array-based status, this will return courses that include the specified status
...
// Helper utility functions for working with course status

/**
 * Check if a course has a specific status
 * @param course - Course object
 * @param status - Status to check
 * @returns boolean
 */
export function courseHasStatus(course: Course, status: CourseStatus): boolean {
  if (Array.isArray(course.status)) {
    return course.status.includes(status);
  }
  return course.status === status;
}

/**
 * Get all statuses for a course
 * @param course - Course object
 * @returns Array of statuses
 */
export function getCourseStatuses(course: Course): CourseStatus[] {
  if (Array.isArray(course.status)) {
...
/**
 * Check if a course is available in multiple modes
 * @param course - Course object
 * @returns boolean
 */
export function isMultiModeCourse(course: Course): boolean {
  return Array.isArray(course.status) && course.status.length > 1;
}
