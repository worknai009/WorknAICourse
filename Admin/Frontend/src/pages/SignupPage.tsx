...
import { Mail, Lock, User, ArrowRight, Loader2, AlertCircle, ShieldPlus } from 'lucide-react';
...
const SignupPage = ({ onSwitch }: { onSwitch: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const res = await axios.post('http://127.0.0.1:5001/api/admin/auth/signup', { name, email, password });
      ...
      <div className="w-full max-w-md space-y-8 bg-[#0a0a0a] p-8 rounded-3xl border border-white/10 shadow-2xl relative overflow-hidden">
        ...
        <div className="text-center space-y-2">
          <div className="flex justify-center">
            <div className="w-16 h-16 bg-purple-600/10 rounded-2xl flex items-center justify-center text-purple-500 border border-purple-500/20 shadow-inner">
                <ShieldPlus size={32} />
            </div>
          </div>
          <h1 className="text-3xl font-black tracking-tighter text-white">Create Admin</h1>
          <p className="text-gray-400 font-medium">Join the WorknAI administration team</p>
        </div>
        ...
        <div className="text-center">
            ...
            >
              Already have an account? <span className="text-blue-500 border-b border-blue-500/30 pb-0.5">Admin Login</span>
            </button>
        </div>
      </div>
    </div>
